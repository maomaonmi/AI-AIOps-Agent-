...
export class UiaBridge {
        ...
        return new Promise((resolve) => {
            try {
                ...
                proc.stderr?.on('data', (c) => {
                    const m = c.trim();
                    if (m && !m.startsWith('WARNING') && !m.startsWith('PS'))
                        console.warn('[uia] stderr:', m.slice(0, 200));
                });
    ...
    send(cmd, ms = 1500) {
        return new Promise((resolve) => {
            if (!this.ps || this.ps.killed || !this.ready) {
                resolve(null);
                return;
            }
            let done = false;
    ...
    drain(v) { while (this.queue.length)
        (this.queue.shift())(v); }
    async poll() {
        const d = await this.send('poll');
        if (!d || d._e || d._s || d._error)
